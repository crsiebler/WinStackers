 ...
 * ========================================================= */!function(e) {
    ...
    var r = function(t, n) {
        ...
        this.language = this.language in i ? this.language : this.language.split("-")[0];
        ...
        this.picker = e(s.template).appendTo(this.isInline ? this.element : "body").on({click: e.proxy(this.click, this), mousedown: e.proxy(this.mousedown, this)});
        ...
        "calendarWeeks"in n ? this.calendarWeeks = n.calendarWeeks : "dateCalendarWeeks"in this.element.data() && (this.calendarWeeks = this.element.data("date-calendar-weeks"));
    ...
    r.prototype = {constructor: r, _events: [], _attachEvents: function() {
            ...
            if (!this.isInput) {
                this.component && this.element.find("input").val(e);
            ...
            this.startDate !== -Infinity && (this.startDate = s.parseDate(this.startDate, this.format, this.language));
            this.update();
            this.updateNavArrows()
        }, setEndDate: function(e) {
            this.endDate = e || Infinity;
            this.endDate !== Infinity && (this.endDate = s.parseDate(this.endDate, this.format, this.language));
            this.update();
            this.updateNavArrows()
            ...
            while (p.valueOf() < v) {
                ...
                if (p.getUTCFullYear() < r || p.getUTCFullYear() == r && p.getUTCMonth() < o)
                    ...
                    g += " new";
                this.todayHighlight && p.getUTCFullYear() == h.getFullYear() && p.getUTCMonth() == h.getMonth() && p.getUTCDate() == h.getDate() && (g += " today");
                c && p.valueOf() == c && (g += " active");
                ...
                p.getUTCDay() == this.weekEnd && m.push("</tr>");
            ...
            S && S == r && x.eq(this.date.getUTCMonth()).addClass("active");
            (r < u || r > f) && x.addClass("disabled");
            r == u && x.slice(0, a).addClass("disabled");
            r == f && x.slice(l + 1).addClass("disabled");
            m = "";
            r = parseInt(r / 10, 10) * 10;
            var T = this.picker.find(".datepicker-years").find("th:eq(1)").text(r + "-" + (r + 9)).end().find("td");
            r -= 1;
            for (var N = -1; N < 11; N++) {
                m += '<span class="year' + (N == -1 || N == 10 ? " old" : "") + (S == r ? " active" : "") + (r < u || r > f ? " disabled" : "") + '">' + r + "</span>";
                r += 1
            }
            T.html(m)
            ...
            switch (this.viewMode) {
                case 0:
                    this.startDate !== -Infinity && t <= this.startDate.getUTCFullYear() && n <= this.startDate.getUTCMonth() ? this.picker.find(".prev").css({visibility: "hidden"}) : this.picker.find(".prev").css({visibility: "visible"});
                    this.endDate !== Infinity && t >= this.endDate.getUTCFullYear() && n >= this.endDate.getUTCMonth() ? this.picker.find(".next").css({visibility: "hidden"}) : this.picker.find(".next").css({visibility: "visible"});
                    break;
                case 1:
                case 2:
                    this.startDate !== -Infinity && t <= this.startDate.getUTCFullYear() ? this.picker.find(".prev").css({visibility: "hidden"}) : this.picker.find(".prev").css({visibility: "visible"});
                    this.endDate !== Infinity && t >= this.endDate.getUTCFullYear() ? this.picker.find(".next").css({visibility: "hidden"}) : this.picker.find(".next").css({visibility: "visible"})
            }
        }, click: function(n) {
            n.stopPropagation();
            n.preventDefault();
            ...
            if (n) {
                n.change();
                this.autoclose && (!t || t == "date") && this.hide()
            }
            ...
            e && (this.viewMode = Math.max(0, Math.min(2, this.viewMode + e)));
    ...
    e.fn.datepicker = function(t) {
        ...
        return this.each(function() {
            ...
            s || i.data("datepicker", s = new r(this, e.extend({}, e.fn.datepicker.defaults, o)));
